...
void init(int mode)
{
	...
	// TODO: add option to choose color scheme
	...
	else if (mode == 4) // big message to the right. Log on Ser
	{
		msg_h = s_row;
		msg_w = s_col - board_w;
		msg_x = 0;
	...
	else if (mode == 5) // big message to the right. Log and Ser side by side
	{
		msg_h = s_row;
		msg_w = s_col - board_w;
		msg_x = 0;
...
void displayStatus()
{
	...
	for (pin = 0; pin < max_digPin; pin++)
	{
		...
		case CHANGE:
			wprintw(uno, "IC ");
			break;
		/* case LOW: // This will never happen since it is a duplicate value of INPUT
			wprintw(uno, "IL ");
			break; */
		case S_WRONG:
			wprintw(uno, "???");
