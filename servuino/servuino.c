...
int main(int argc, char *argv[])
{
	...
	strcpy(g_version, "0.1.3");

	openFiles();
	readSketchInfo();
	g_nTotPins = setRange(g_boardType);
	if (g_boardType == UNO)
		g_nDigPins = MAX_PIN_DIGITAL_UNO;
	...
	else if (argc == 8)
	{
		...
		{
			if (g_action == S_ADD)
				x = insDigitalPinValue(g_pinNo, g_pinStep, g_pinValue);
			else if (g_action == S_DELETE)
				x = delDigitalPinValue(g_pinNo, g_pinStep);
		}
		else if (g_pinType == ANA)
		{
			if (g_action == S_ADD)
				x = insAnalogPinValue(g_pinNo, g_pinStep, g_pinValue);
			else if (g_action == S_DELETE)
				x = delAnalogPinValue(g_pinNo, g_pinStep);
		}
		runEncoding(g_simulationLength);
	}
	else
		errorLog("Servuino not executed; Invalid arguments", 0);
